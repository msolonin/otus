...
def test_update_add_authorized_session(base_url):
    ...
    # Authorization
    response_before = requests.get(base_url + "/auth/status")
    session.request("login", base_url + "/auth/login", json=AUTH_DATA)
    response_after = requests.get(base_url + "/auth/status")
    session_response_after = session.get(base_url + "/auth/status")
    ...
    assert cerberus_validator.validate(data, schema)
